...
from tensorflow.keras.applications import VGG16,VGG19, ResNet50,InceptionV3,Xception,ResNet50V2,DenseNet121,EfficientNetB0,InceptionResNetV2 #EfficientNetV2B0
import logging
import time
...
class Models:
    ...
    """
    MODELS = [VGG16,VGG19,ResNet50,Xception,ResNet50V2,InceptionV3,DenseNet121,EfficientNetB0,InceptionResNetV2] # VGG19,ResNet50,Xception,ResNet50V2,InceptionV3,DenseNet121,EfficientNetB0,InceptionResNetV2
    ...
    def train(self,X_train: np.ndarray,X_test: np.ndarray,y_train: np.ndarray,y_test: np.ndarray,input_shape: tuple,output_nums: int,CLASSES: list,epochs=10, dataset="dataset", batch_size=32):
        """
        ...
        for j in  tqdm(range(len(self.MODELS))):
            ...
            logging.info("====================================================================")
            start = time.time()
            _ =  model.fit(x = X_train, y =y_train, epochs=epochs, validation_split = 0.1, batch_size=batch_size )
            stop = time.time()
            y_pred = np.argmax(model.predict(X_test), axis = 1)
            hist['model'].append(f"{model.name}")
            ...
            Utils.plot_confusion_matrix(y_test, y_pred,CLASSES, save=True,filename=f"{dataset}_{model.name}_plot_confusion_matrix" )
            model.save(f'./Models/{dataset}_{model.name}.keras')
            logging.info(f"{model.name} trained and evaluted successfully.")
        pd.DataFrame(hist).to_csv(f'./Data/{dataset}_Evaluation_Results.csv')
        logging.info(f"Evaluation results written to ./Data/{dataset}_Evaluation_Results.csv")
